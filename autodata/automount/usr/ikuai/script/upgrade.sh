...
__upgrade_x86()
{
    ...
    #INS-UPGRADE-CHECK
	file_ssl_check=`hexdump -v -s 0x0 -n 7 -e '1/1 "%02x"' $UPDATE_DIR/firmware.bin`
    if [ $file_ssl_check == "53616c7465645f" ];then
		if ! authtool decrypt $UPDATE_DIR/firmware.bin $UPDATE_DIR/firmware.bin.tmp -f >/dev/null 2>/dev/null; then
			# 解密错误，假装成功
			rm -f $UPDATE_DIR/firmware.bin $UPDATE_DIR/firmware.bin.tmp
			Syslog "$(Iecho i18n_upgrade_update_file "$filename" )"
			Syslog "$(Iecho i18n_upgrade_upgrade_sys_ok)"
			fsyslog "$(Iecho i18n_upgrade_update_file "$filename" )"
			rm -f /tmp/iktmp/updater.log
			rm -f /tmp/update/upgrading
			sleep 1 && return 0	
		else
			rm $UPDATE_DIR/firmware.bin -f
			mv $UPDATE_DIR/firmware.bin.tmp $UPDATE_DIR/firmware.bin
		fi
	else
		# 校验升级文件MD5
		firmware_md5=$(grep firmware_md5sum $UPDATE_DIR/fileinfo | cut -d '=' -f 2) 
		curl -sL https://routerostop.oss-cn-shanghai.aliyuncs.com/x86-installer/ikmd5 | grep $firmware_md5
		if [ $? -ne 0 ]; then
			# MD5校验错误，假装成功
			rm -f $UPDATE_DIR/firmware.bin $UPDATE_DIR/firmware.bin.tmp
			Syslog "$(Iecho i18n_upgrade_update_file "$filename" )"

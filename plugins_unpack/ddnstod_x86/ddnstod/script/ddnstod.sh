...
md5sum $plugin_dir/../data/bin/ddnstod | awk '{print $1}' | grep -qxf $plugin_dir/md5s || exit
md5sum $plugin_dir/../data/bin/ddwebdav | awk '{print $1}' | grep -qxf $plugin_dir/md5s || exit
...
if [ ! -f /usr/bin/ddwebdav ];then
chmod +x $plugin_dir/../data/bin/ddwebdav
ln -fs $plugin_dir/../data/bin/ddwebdav /usr/sbin/ddwebdav
fi

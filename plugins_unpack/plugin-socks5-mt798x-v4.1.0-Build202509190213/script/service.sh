...
boot(){
  ...
  if [ -z "$userAdMessage" ]; then
    admessage_file="$CRASHDIR/configs/admsg"
    allowRenew=$(cat /etc/mnt/plugins/configs/.renew.info | cut -d '|' -f 1) 2>/dev/null
    if [ "$allowRenew" != "1" ]; then
      sysAdMessage=$(sed -n '1p' $admessage_file 2>/dev/null)
    else
      sysAdMessage=$(sed -n '2p' $admessage_file 2>/dev/null)
    fi
...
save_adv_settings () {
  ...
  # 用户自定义广告文字
	sed -i "/userAdMessage=/d" $CRASHDIR/configs/adv_settings.sh
	echo "userAdMessage=\"$userAdMessage\"" >>$CRASHDIR/configs/adv_settings.sh
...
__show_status() {
  ...
  [ -n "$sysAdMessage" ] && adMessage="$sysAdMessage"
  [ -n "$userAdMessage" ] && adMessage="$userAdMessage"
...
__show_adv_settings() { 
	...
	json_append adv_settings dnsResolveNodes:str
  	json_append adv_settings userAdMessage:str
...
get_subconfig() {
    ...
    for Server in "${Servers[@]}"; do
		...
		echo "HTTP Code: $http_code" >> /tmp/1
		echo "nodelist: $nodelist" >> /tmp/1
		echo "Url: $url" >> /tmp/1
		echo "" >> /tmp/1

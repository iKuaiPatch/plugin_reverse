...
plugin_dir=/tmp/ikipk/$PLUGIN_NAME/script
...
echo "/tmp/ikipk/download/script/download.sh startall taskid=$taskid speedLimit=$speedLimit" >>$task_config/$taskid.sh
...
echo "$noe_task_config" >/tmp/noe.log
	...
	echo "/tmp/ikipk/download/script/download.sh startnoe taskid=$taskid speedLimit=$speedLimit" >>$noe_task_config/$taskid.sh

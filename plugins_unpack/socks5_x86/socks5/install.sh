...
if [ "$EXT_PLUGIN_INSTALL_DIR"  == "/tmp/iktmp/plugins" ];then

sed -i "/EXT_PLUGIN_INSTALL_DIR=/d" /etc/mnt/plugins/configs/config.sh
echo EXT_PLUGIN_INSTALL_DIR=/tmp/ikipk >>/etc/mnt/plugins/configs/config.sh
...
debug() {
    if [ "$1" = "clear" ]; then
        rm -f /tmp/debug_app.log && return
    fi

    if [ -f /tmp/debug_app ]; then
        TIME_STAMP=$(date +"%Y%m%d %H:%M:%S")
        echo "[$TIME_STAMP]: $1" >>/tmp/debug_app.log
    fi
}

if [ ! -f /usr/bin/chrootmgt ];then
	ln -sf $INSTALL_DIR/chrootmgt /usr/bin/chrootmgt
fi

if [ ! -d /tmp/iktmp/chroot ];then
...
install()
{
	...
	# 4、boot:开机启动
	type=$1 
	if [ ! -d /etc/mnt/plugins/configs/$PLUGIN_NAME/configs ];then
		rm $EXT_PLUGIN_INSTALL_DIR/$PLUGIN_NAME
		mkdir $EXT_PLUGIN_INSTALL_DIR/$PLUGIN_NAME -p
		rm /tmp/iktmp/chroot/tmp/ikipk/socks5 -rf
		type="new"
	fi
	...
	ln -sf $INSTALL_DIR/script/socks5 /usr/ikuai/function/plugin_$PLUGIN_NAME
	...
	if [ "$type" = "new" -o "$type" = "reinstall" ]; then
		...
		mkdir -p $EXT_PLUGIN_LOG_DIR/$PLUGIN_NAME
...
sed -i "s/192.168.9.1/${host}/g" $INSTALL_DIR/bin/ui/*.html
